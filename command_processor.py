...
# MODIFIED: Added /profile, /profile_for_npc commands and enhanced action tracking for profile updates.
...
import hashlib
import time
from typing import Dict, List, Any, Optional, Tuple, Callable # Added Callable for llm_wrapper

import session_utils
try:
    import hint_manager
    from player_profile_manager import get_distilled_profile_insights_for_npc # For /profile_for_npc
except ImportError:
...
# --- Utility to add action to profile tracking ---
...
def handle_help(state: Dict[str, Any]) -> HandlerResult:
    ...
    try:
        ...
        print(get_help_text()) # Ensure this is updated for new profile commands
    except ImportError:
...
def handle_hint(state: Dict[str, Any]) -> HandlerResult:
    TF = state['TerminalFormatter']; player_id = state['player_id']; db = state['db']
    ChatSession_class = state['ChatSession']; llm_wrapper_func = state.get('llm_wrapper_func')
    _add_profile_action(state, "Used /hint command")

    if not llm_wrapper_func: # Crucial check
        print(f"{TF.RED}LLM wrapper function not available in game state. Advanced hint cannot proceed.{TF.RESET}")
        # Fallback to simple hint if possible
        ...
        else: print(f"{TF.YELLOW}Not in a regular conversation to get a simple hint, or LLM for advanced hint is unavailable.{TF.RESET}")
        return {**state, 'status': 'ok', 'continue_loop': True}
    ...
    current_cache_key = hint_manager.generate_cache_key(state) # Includes profile hash
    ...
    player_profile = state.get('player_profile_cache', {}) # Get current profile
    ...
    if cached_hint_data and cached_hint_data.get('key') == current_cache_key and \
       (time.time() - cached_hint_data.get('timestamp', 0)) < 300: # 5 min cache
        ...
        # System prompt for Lyra during hint needs to be rebuilt with current profile context
        lyra_system_prompt = session_utils.build_system_prompt(
            lyra_npc_data_from_db, state['story'], TF, player_id, db,
            player_profile=player_profile, # Ensure Lyra's sys prompt gets the profile
            llm_wrapper_func=llm_wrapper_func, llm_model_name=state['model_name']
            # No conversation_summary_for_lyra needed when restoring a cached *response*
        )
        ...
        else: print(f"{TF.YELLOW}Could not recall Lyra's last specific advice for this exact state, preparing fresh consultation.{TF.RESET}")
    ...
    if stashed_session_temp and stashed_session_temp.messages and llm_wrapper_func:
        ...
        except Exception as e: conversation_summary = f"[Error during summarization: {e}]"

    player_state_data = db.load_player_state(player_id) or {}
    player_data_for_lyra = {
        'inventory': state.get('player_inventory', []), 'credits': state.get('player_credits_cache', 0),
    ...
    initial_lyra_user_prompt = hint_manager.build_initial_lyra_prompt( # This is the "USER" message to Lyra
    ...
    # Lyra's SYSTEM prompt needs profile, and also the summary of the stashed conversation
    lyra_system_prompt = session_utils.build_system_prompt(
        lyra_npc_data_from_db, state['story'], TF, player_id, db,
        conversation_summary_for_lyra=conversation_summary, # This goes into her SYSTEM prompt
    ...
    try:
        ...
        if state.get('auto_show_stats', False) and stats and state.get('format_stats'): print(state['format_stats'](stats))
    except Exception as e: print(f"{TF.RED}Error during Lyra consultation: {e}{TF.RESET}"); return {**state, 'status': 'ok', 'continue_loop': True}

    state['lyra_hint_cache'] = {'key': current_cache_key, 'lyra_chat_history': lyra_hint_session_new.get_history(), 'timestamp': time.time()}
    state['stashed_chat_session'] = stashed_session_temp; state['stashed_npc'] = stashed_npc_temp
    state['in_lyra_hint_mode'] = True; state['current_npc'] = lyra_npc_data_from_db; state['chat_session'] = lyra_hint_session_new
...
def handle_endhint(state: Dict[str, Any]) -> HandlerResult:
    ...
    if not state.get('in_lyra_hint_mode'):
        ...
        return {**state, 'status': 'ok', 'continue_loop': True}
    restored_npc = state.get('stashed_npc'); restored_session = state.get('stashed_chat_session')
    state['in_lyra_hint_mode'] = False; state['current_npc'] = restored_npc; state['chat_session'] = restored_session
    state['stashed_npc'] = None; state['stashed_chat_session'] = None
    if restored_npc and restored_session:
        ...
        if restored_session.messages:
            ...
            # Display logic for resuming conversation...
        ...
        state['current_npc'] = None; state['chat_session'] = None
...
def handle_inventory(state: Dict[str, Any]) -> HandlerResult:
    ...
    if inventory_list: [print(f"  {TF.GREEN}❖ {item.title()}{TF.RESET}") for item in inventory_list]
    else: print(f"  {TF.DIM}(Your inventory is empty){TF.RESET}")
...
def handle_give(args: str, state: Dict[str, Any]) -> HandlerResult:
    ...
    if not input_str: print(f"{TF.YELLOW}What do you want to give? Usage: /give <item_name_or_amount Credits>{TF.RESET}"); return {**state, 'status': 'ok', 'continue_loop': True}
    ...
    if credit_match:
        ...
        if amount_to_give <= 0: print(f"{TF.YELLOW}You must give a positive amount of Credits.{TF.RESET}"); return {**state, 'status': 'ok', 'continue_loop': True}
        if player_current_credits >= amount_to_give:
            if db.update_player_credits(player_id, -amount_to_give, state):
                ...
                command_result_payload['item_given_to_npc_this_turn'] = {'item_name': item_name_for_log_and_action, 'type': 'currency', 'amount': amount_to_give, 'npc_code': current_npc.get('code')}
        else: print(f"{TF.YELLOW}You don't have enough Credits. You only have {player_current_credits}.{TF.RESET}"); return {**state, 'status': 'ok', 'continue_loop': True}
    else:
        ...
        if not db.check_item_in_inventory(player_id, cleaned_item_name_for_check):
            print(f"{TF.YELLOW}You don't have '{item_name_to_give}' (or similar) in your inventory.{TF.RESET}"); return {**state, 'status': 'ok', 'continue_loop': True}
        if db.remove_item_from_inventory(player_id, cleaned_item_name_for_check, state):
            ...
            command_result_payload['item_given_to_npc_this_turn'] = {'item_name': cleaned_item_name_for_check, 'original_user_input_item_name': item_name_for_log_and_action, 'type': 'item', 'npc_code': current_npc.get('code')}
        else: print(f"{TF.RED}Error: Could not remove '{item_name_to_give}' from inventory (unexpected).{TF.RESET}"); return {**state, 'status': 'ok', 'continue_loop': True}

    if item_name_for_log_and_action: # If something was successfully processed to be given
...
def handle_go(args: str, state: Dict[str, Any]) -> HandlerResult:
    ...
    if state.get('in_lyra_hint_mode'): print(f"{TF.YELLOW}You cannot use /go while consulting Lyra. Use /endhint first.{TF.RESET}"); return {**state, 'status': 'ok', 'continue_loop': True}
    # ... (rest of /go logic is mostly fine)
    ...
    target_terms = args.lower().split(); all_known_npcs = session_utils.refresh_known_npcs_list(db, TF)
    known_areas = session_utils.get_known_areas_from_list(all_known_npcs); matches = []
    if target_terms: matches = [area_name_full for area_name_full in known_areas if all(term in area_name_full.lower() for term in target_terms)]
    ...
    if len(matches) == 1:
        ...
        if current_area_from_state is not None and current_area_from_state.lower() == area.lower():
            print(f"{TF.DIM}You are already in {area}.{TF.RESET}"); return {**state, 'status': 'ok', 'continue_loop': True}
        ...
        llm_wrapper_for_npc_setup = state.get('llm_wrapper_func') # Get from main state
        npc_data, session = session_utils.auto_start_default_npc_conversation(
            ...
            llm_wrapper_for_profile_distillation=llm_wrapper_for_npc_setup # Pass for profile in new NPC
        )
        if npc_data and session: new_state_part['current_npc'] = npc_data; new_state_part['chat_session'] = session
...
def handle_talk(args: str, state: Dict[str, Any]) -> HandlerResult:
    ...
    if state.get('in_lyra_hint_mode'): print(f"{TF.YELLOW}You cannot use /talk while consulting Lyra. Use /endhint first.{TF.RESET}"); return {**state, 'status': 'ok', 'continue_loop': True}
    # ... (rest of /talk logic is mostly fine)
    ...
    target_npc_info = None; talk_initiated = False; new_state_part: Dict[str, Any] = {}
    all_npcs = session_utils.refresh_known_npcs_list(db, TF); npcs_here = [n for n in all_npcs if n.get('area','').lower() == area.lower()]
    if args == RANDOM_TALK_SYNTAX: # ... (random talk logic)
        if not npcs_here: print(f"{TF.YELLOW}No NPCs in {area}.{TF.RESET}"); return state
        eligible_npcs = [n for n in npcs_here if not (npc_old and n.get('code') == npc_old.get('code'))]
        if not eligible_npcs and npc_old : print(f"{TF.DIM}Only {npc_old.get('name')} is here, and you're already talking to them.{TF.RESET}"); return state
        elif not eligible_npcs and not npc_old: print(f"{TF.YELLOW}No other NPCs in {area} to talk to randomly.{TF.RESET}"); return state
        target_npc_info = random.choice(eligible_npcs if eligible_npcs else npcs_here)
        print(f"{TF.DIM}Randomly approaching: {target_npc_info['name']}{TF.RESET}"); talk_initiated = True
    else: # ... (specific NPC talk logic)
        prefix = args.lower(); matches = [n for n in npcs_here if n.get('name','').lower().startswith(prefix)]
        if len(matches) == 1: target_npc_info = matches[0]; talk_initiated = True
    ...
    if talk_initiated and target_npc_info:
        if npc_old and npc_old.get('code') == target_npc_info.get('code'): print(f"{TF.DIM}You are already talking to {target_npc_info['name']}.{TF.RESET}")
        else:
            if session_old:
            ...
            npc_data, new_session = session_utils.start_conversation_with_specific_npc(
                ...
                llm_wrapper_for_profile_distillation=llm_wrapper_for_npc_setup # Pass for profile
            )
            if npc_data and new_session: new_state_part.update({'current_npc': npc_data, 'chat_session': new_session})
            else: new_state_part.update({'current_npc': None, 'chat_session': None}); print(f"{TF.RED}Error initiating talk with {target_npc_info.get('name', 'NPC')}.{TF.RESET}")
    state.update(new_state_part)
    return {**state, 'status': 'ok', 'continue_loop': True}
...
def handle_who(state: Dict[str, Any]) -> HandlerResult:
    ...
    if not current_display_area: print(f"{TF.YELLOW}Not in an area. Use /go.{TF.RESET}"); return{**state,'status':'ok','continue_loop':True}
    ...
    if not npcs_here:print(f"\n{TF.YELLOW}No NPCs in {current_display_area}.{TF.RESET}")
        ...
        for n in sorted(npcs_here,key=lambda x:x.get('name','').lower()): print(f"  {TF.DIM}- {n.get('name','???')} ({n.get('role','???')}){TF.RESET}")
    return{**state,'status':'ok','continue_loop':True}

def handle_whereami(state: Dict[str, Any]) -> HandlerResult:
    TF=state['TerminalFormatter']
    _add_profile_action(state, "Used /whereami command")
    # ... (/whereami logic is fine)
    ...
    display_area = current_location_area; display_npc_name = "Nobody"; hint_for_display = None
    if is_hint_mode:
        stashed_npc_data = state.get('stashed_npc')
        stashed_area = stashed_npc_data.get('area', "Unknown (stashed)") if stashed_npc_data else "Unknown (stashed)"
        display_area = f"{stashed_area} (Physically)"; lyra_name = current_talking_to_npc.get('name', 'Lyra') if current_talking_to_npc else 'Lyra'
        ...
        if current_talking_to_npc:
            ...
            if current_chat_session_obj: hint_for_display = current_chat_session_obj.get_player_hint()
    loc_msg = f"Location: {TF.BOLD}{display_area}{TF.RESET}"; talking_msg = f"Talking to: {TF.BOLD}{display_npc_name}{TF.RESET}"
    if hint_for_display and not is_hint_mode: talking_msg += f"\n  {TF.DIM}(Hint for {display_npc_name}: {hint_for_display[:70]}{'...'if len(hint_for_display)>70 else''}){TF.RESET}"
    print(f"\n{TF.CYAN}{loc_msg}\n{talking_msg}{TF.RESET}")
    return{**state,'status':'ok','continue_loop':True}


def handle_npcs(state: Dict[str, Any]) -> HandlerResult: # List all NPCs
    TF = state['TerminalFormatter']; db = state['db']
...
def handle_areas(state: Dict[str, Any]) -> HandlerResult: # List all areas
    TF = state['TerminalFormatter']; db = state['db']
    ...
    if not known_areas: print(f"{TF.YELLOW}No known areas found.{TF.RESET}")
    else:
        print(f"\n{TF.BRIGHT_CYAN}{TF.BOLD}Available Areas to /go to:{TF.RESET}")
        for area_name in known_areas: print(f"  {TF.CYAN}➢ {area_name}{TF.RESET}")
    return {**state, 'status': 'ok', 'continue_loop': True}
...
def handle_profile(state: Dict[str, Any]) -> HandlerResult:
    ...
    if profile_data:
        ...
        if patterns: [print(f"  {TF.DIM}- {p.replace('_', ' ').capitalize()}{TF.RESET}") for p in patterns]
        else: print(f"  {TF.DIM}(None observed yet){TF.RESET}")
        ...
        if experiences: [print(f"  {TF.DIM}- {exp.replace('_', ' ').capitalize()}{TF.RESET}") for exp in experiences]
        else: print(f"  {TF.DIM}(None recorded yet){TF.RESET}")

        print(f"\n{TF.YELLOW}Trust Levels:{TF.RESET}") # Could be expanded for NPC-specific trust
        ...
        if motivations: [print(f"  {TF.DIM}- {m.replace('_', ' ').capitalize()}{TF.RESET}") for m in motivations]
        else: print(f"  {TF.DIM}(Primary quest focus assumed){TF.RESET}")
    else:
...
def handle_profile_for_npc(state: Dict[str, Any]) -> HandlerResult:
    ...
    print(f"\n{TF.BRIGHT_CYAN}{TF.BOLD}--- Distilled Profile Insights for {npc_name} ---{TF.RESET}")
...
# ... (other handlers like /stats, /clear, /history are mostly fine but add _add_profile_action)
def handle_stats(state: Dict[str, Any]) -> HandlerResult:
    ...
    ls=session.get_last_stats(); npc_name_for_stats = npc.get('name', 'Current NPC')
    if state.get('in_lyra_hint_mode') and npc_name_for_stats.lower() == 'lyra': npc_name_for_stats = "Lyra (Consultation)"
...
def handle_clear(state: Dict[str, Any]) -> HandlerResult:
    ...
    if not session:print(f"{TF.YELLOW}Not in an active chat to clear.{TF.RESET}")
        ...
        session.clear_memory(); npc_name = state.get('current_npc',{}).get('name', 'current')
...
def process_input_revised(user_input: str, state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not user_input.strip(): return state # No action for empty input
    ...
    if user_input.startswith('/'):
        ...
        command_handlers_map = {
            ...
            'profile': handle_profile, 'profile_for_npc': handle_profile_for_npc # New commands
        }
        try:
            if command in command_handlers_map:
                handler_func = command_handlers_map[command]
                # Commands that take args vs those that don't
                if command in ['go', 'talk', 'give']: state = handler_func(args_str, state)
                else: state = handler_func(state)
                if state.get('status') == 'exit': return state
                command_processed_this_turn = True # Assume command was processed if handled
            elif command == 'history': # Special case not needing args_str
                ...
                command_processed_this_turn = True # Still counts as a processed command (an invalid one)
        except Exception as e:
            print(f"{TF.RED}Error processing command '/{command}': {type(e).__name__} - {e}{TF.RESET}")
            traceback.print_exc(); command_processed_this_turn = True
    else: # Not a command, so it's dialogue for the NPC
    ...
    if force_npc_turn_after_command: state.pop('force_npc_turn_after_command', None) # Consume the flag

    needs_llm_call = False
    prompt_for_llm = user_input

    if not command_processed_this_turn: needs_llm_call = True
    elif force_npc_turn_after_command and not is_in_lyra_hint_mode: # e.g. after /give
        prompt_for_llm = "[NPC reacts to player's action]" # This prompt is added to history by chat_session.ask
        needs_llm_call = True

    # If in Lyra hint mode, any non-command input is a follow-up question for Lyra
    ...
    if needs_llm_call:
        if current_npc and chat_session:
            ...
            try:
                ...
                if _response_text.strip(): # Log NPC response for profile if not empty
                    _add_profile_action(state, f"NPC Response from {npc_name_for_prompt}: '{_response_text[:50]}{'...' if len(_response_text) > 50 else ''}'")

                if state.get('auto_show_stats', False) and stats and fmt_stats_func: print(fmt_stats_func(stats))
                if is_in_lyra_hint_mode and state.get('lyra_hint_cache'): # Update Lyra's cache with new interaction
                    state['lyra_hint_cache']['lyra_chat_history'] = chat_session.get_history()
                    state['lyra_hint_cache']['timestamp'] = time.time()
            except Exception as e:
                state['npc_made_new_response_this_turn'] = False
                print(f"{TF.RED}LLM Chat Error with {npc_name_for_prompt}: {type(e).__name__} - {e}{TF.RESET}"); traceback.print_exc()
        elif user_input: # User typed dialogue but not in a conversation
