...
def handle_exit(state: Dict[str, Any]) -> HandlerResult:
    ...
    session_utils.save_current_conversation(
        state['db'], state['current_npc'], state['chat_session'], TerminalFormatter
    )
...
def handle_go(args: str, state: Dict[str, Any]) -> HandlerResult:
    ...
    if len(matches) == 1:
        ...
        print(f"{TerminalFormatter.YELLOW}You are now in {matched_area}. Use {TerminalFormatter.BOLD}/talk <npc_name_or_prefix|.>{TerminalFormatter.YELLOW} or {TerminalFormatter.BOLD}/who{TerminalFormatter.YELLOW}.")
...
def handle_talk(args: str, state: Dict[str, Any]) -> HandlerResult:
    ...
    if args == RANDOM_TALK_SYNTAX:
        ...
        if len(matches) == 1:
            ...
            print(f"{TerminalFormatter.DIM}Unique match found: {target_npc_info['name']}{TerminalFormatter.RESET}")
            initiate_talk = True
        elif len(matches) > 1:
    ...
    if initiate_talk and target_npc_info:
        if current_npc and current_npc.get('code') == target_npc_info.get('code'):
            ...
            if npc_data and new_session:
                ...
                print(f"{TerminalFormatter.RED}❌ Failed to start conversation with {correct_npc_name}. Check logs.{TerminalFormatter.RESET}")
...
def process_input_revised(user_input: str, state: Dict[str, Any]) -> HandlerResult:
    ...
    # Handle normal dialogue (if in active conversation)
        ...
        try:
            ...
            print(f"{TerminalFormatter.BOLD}{TerminalFormatter.MAGENTA}{npc_name} > {TerminalFormatter.RESET}")
            print(TerminalFormatter.format_terminal_text(response,
    ...
    # Not in a conversation and not a command
    else:
        print(f"{TerminalFormatter.YELLOW}You are not currently talking to anyone. Use /go to navigate to an area, then /talk to speak with an NPC.{TerminalFormatter.RESET}")
