...
def normalize_text_for_lsl(text):
    ...
    Also strips inline SL command tags [emote=...; anim=...; ...] that the LLM
    may embed in the response text — these are for the LSL script, not the chat.
    ...
    import re as _re
    # Strip inline SL command blocks: [emote=...; anim=...; facial_expression=...; ...]
    # These are generated by the LLM and must not appear in the displayed chat text.
    text = _re.sub(r'\[(?:emote|anim|face|facial_expression|lookup|llSetText|teleport|notecard)[^\]]*\]',
                   '', text, flags=_re.IGNORECASE)
    # Clean up any double spaces/newlines left after stripping
    text = _re.sub(r'\n{3,}', '\n\n', text)
    text = text.strip()
