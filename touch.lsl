...
string NPC_NAME = "SimpleNPC";                // Name of this NPC
...
default
{
    state_entry()
    {
        ...
        llOwnerSay("Touch-activated NPC initialized. Waiting for touch.");
    }
    ...
    touch_start(integer total_number)
    {
        ...
        else
        {
            ...
            llRegionSayTo(current_toucher, 0, "Ciao! Sono " + NPC_NAME + ". Puoi parlare con me ora.");
            llSay(0, "Ciao! Sono " + NPC_NAME + ". Cosa vuoi sapere?");
    ...
    listen(integer channel, string name, key id, string message)
    {
        ...
        if (IS_CONVERSING && id == current_toucher)
        {
            ...
            call_chat_endpoint(message, name);
        }
    }
    ...
    timer()
    {
        ...
        if (IS_CONVERSING)
        {
            ...
            end_conversation();
            llRegionSayTo(current_toucher, 0, "Il tempo per la conversazione è finito. Toccami di nuovo per continuare a parlare.");
        }
    }
    ...
    http_response(key request_id, integer status, list metadata, string body)
    {
        ...
        if (request_id == chat_request_id && IS_CONVERSING)
        {
            ...
            else
            {
                ...
                llRegionSayTo(current_toucher, 0, "Scusa, al momento non posso risponderti.");
...
call_leave_endpoint(string avatar_name)
{
    ...
    leave_request_id = llHTTPRequest(SERVER_URL + "/leave", http_options, json_data);
}
...
end_conversation()
{
    if (IS_CONVERSING)
    {
        ...
        if (current_toucher != NULL_KEY)
        {
            llRegionSayTo(current_toucher, 0, "È stato un piacere parlare con te, " + current_toucher_name + "! A presto.");
        }
