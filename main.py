...
def main():
  ...
  parser.add_argument(
    "--area", default="Sanctum of Whispers",  # FIXED: Default start in Sanctum
    help="Attempt to start in this area (case-insensitive)."
  )
  parser.add_argument(
    "--npc", default="Lyra",  # FIXED: Default start with Lyra
  ...
  parser.add_argument(
    "--debug", action="store_true",  # FIXED: Added debug flag
  ...
  DEBUG_MODE = args.debug  # FIXED: Pass debug flag
  ...
  try:
    run_interaction_loop(
      ...
      debug_mode=DEBUG_MODE,  # FIXED: Pass debug flag
      player_id=PLAYER_ID
    )
  except Exception as e:
