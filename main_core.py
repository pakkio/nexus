...
try:
    ...
    from chat_manager import ChatSession, format_stats # source [378]
    ...
    class TerminalFormatter:
        RED = ""; RESET = ""; BOLD = ""; YELLOW = ""; DIM = ""; # source [379]
        MAGENTA = "" # source [380]
        CYAN = ""; BRIGHT_CYAN = ""; BG_GREEN = ""; BLACK = ""; BRIGHT_MAGENTA = "" # source [381]
        @staticmethod
        def format_terminal_text(text, width=80): import textwrap; return "\n".join(textwrap.wrap(text, width=width)) # source [382]
        @staticmethod
...
try:
    ...
    try:
        ...
        # Define minimal fallback if TerminalFormatter failed *and* was needed for this error msg # source [383]
        class TF: # Corrected: Class definition on new line
            RED = ""; RESET = "" # source [384]
...
def run_interaction_loop(
    ...
    if initial_area:
        print(f"{TerminalFormatter.DIM}Validating initial area '{initial_area}'...{TerminalFormatter.RESET}") # source [386]
        ...
        if len(matches) == 1:
            ...
            print(f"{TerminalFormatter.YELLOW}⚠️ Initial area prefix '{initial_area}' is ambiguous. Matches:{TerminalFormatter.RESET}") # source [387]
    ...
    if not current_area and not initial_area:
        print(f"\n{TerminalFormatter.YELLOW}No starting area specified. Use {TerminalFormatter.BOLD}/go <area_name_or_prefix>{TerminalFormatter.YELLOW} to begin.{TerminalFormatter.RESET}") # source [388]
    ...
    if initial_npc_name and current_area:
        ...
        matches = [n for n in npcs_in_initial_area if n.get('name','').lower().startswith(initial_npc_prefix)] # source [389]
        ...
        if len(matches) == 1:
            ...
            npc_data, session = session_utils.load_and_prepare_conversation( # source [390]
                db, player_id, current_area, correct_npc_name, model_name, story, ChatSession, TerminalFormatter # MODIFIED
            )
            if npc_data and session:
                current_npc = npc_data
                chat_session = session # source [391]
                ...
                print(f"{TerminalFormatter.YELLOW}⚠️ Failed to load conversation data for initial NPC {correct_npc_name}.{TerminalFormatter.RESET}") # source [393]
        elif len(matches) > 1:
            print(f"{TerminalFormatter.YELLOW}⚠️ Initial NPC prefix '{initial_npc_name}' is ambiguous in {current_area}. Matches:{TerminalFormatter.RESET}") # source [394]
        ...
        print(f"\n{TerminalFormatter.YELLOW}Warning: Initial NPC specified ({initial_npc_name}) but the initial area was invalid or ambiguous. Use /go and /talk.{TerminalFormatter.RESET}") # source [395]
    ...
    while True:
        try:
            ...
            current_state = { # source [397]
                'db': db,
                'story': story,
                'current_area': current_area,
                'current_npc': current_npc,
                'chat_session': chat_session,
                'model_name': model_name, # source [398]
                'use_stream': use_stream,
                'auto_show_stats': auto_show_stats,
                'player_id': player_id,
                # Pass class/function references needed by handlers/utils
                'ChatSession': ChatSession, # Pass the actual ChatSession class # source [399]
            ...
            handler_result = command_processor.process_input_revised(user_input, current_state) # source [400]
            ...
            # Use .get(key, default) to avoid errors if key is missing # source [401]
            ...
            if not handler_result.get('continue_loop', True): # source [402]
                # This might be used by a future command that needs to break differently
                break

        # --- Main Loop Exception Handling ---
        except KeyboardInterrupt:
            print(f"\n{TerminalFormatter.DIM}Interruption detected. Saving last conversation (if active)...{TerminalFormatter.RESET}") # source [403]
            ...
            session_utils.save_current_conversation(db, player_id, current_npc, chat_session, TerminalFormatter) # MODIFIED # source [404]
            print(f"{TerminalFormatter.YELLOW}\nExiting.{TerminalFormatter.RESET}")
            break
        except Exception as e:
            print(f"\n{TerminalFormatter.RED}❌ Unexpected Error in Main Loop: {e}{TerminalFormatter.RESET}")
            traceback.print_exc()
            # session_utils.save_current_conversation(db, player_id, current_npc, chat_session, TerminalFormatter) # Optional save, MODIFIED
            print(f"{TerminalFormatter.YELLOW}Attempting to continue... (Restart may be needed){TerminalFormatter.RESET}") # source [405]

    # --- End of Session Cleanup ---
    print(f"\n{TerminalFormatter.BRIGHT_CYAN}{'=' * 50}{TerminalFormatter.RESET}")
    if chat_session and current_npc:
        print(f"{TerminalFormatter.YELLOW}Session terminated. Final stats for ({current_npc.get('name', 'N/A')}):{TerminalFormatter.RESET}") # source [406]
