...
def run_interaction_loop(
    ...
    debug_mode: bool,  # FIXED: Added debug_mode parameter
    player_id: str
):
  ...
  game_session_state: Dict[str, Any] = {
    ...
    'debug_mode': debug_mode,  # FIXED: Store debug mode in state
    'player_id': player_id,
  ...
  if initial_area:
    ...
    if len(area_matches) == 1:
      ...
      print(f"{TerminalFormatter.YELLOW}⚠️ Initial area '{initial_area}' ambiguous.{TerminalFormatter.RESET}")
    else: 
      print(f"{TerminalFormatter.YELLOW}⚠️ Initial area '{initial_area}' not found.{TerminalFormatter.RESET}")
  elif initial_npc_name and not initial_area: 
    print(f"{TerminalFormatter.YELLOW}Warn: --npc specified without --area.{TerminalFormatter.RESET}")

  if not game_session_state['current_npc'] and not game_session_state['current_area']:
    print(f"\n{TerminalFormatter.BRIGHT_CYAN}Welcome to Eldoria! Starting your journey...{TerminalFormatter.RESET}")
    print(f"{TerminalFormatter.DIM}Beginning in the Sanctum of Whispers with Lyra, your guide.{TerminalFormatter.RESET}")
  ...
  while True:
    try:
      ...
      game_session_state['actions_this_turn_for_profile'] = []
        ...
        if raw_npc_response_text and current_npc_data and db_instance and player_id_curr:
          ...
          if tag_start_idx != -1:
            ...
            if tag_end_idx != -1:
              ...
              # FIXED: Only give items if debug or special conditions met
              if debug_mode:
      ...
      if item_given_details and isinstance(item_given_details, dict) and \
        ...
        if debug_mode:  # FIXED: Only show in debug mode
      ...
      if (game_session_state.get('npc_made_new_response_this_turn') or item_given_details or user_input.startswith('/')) and \
         ...
         game_session_state.get('actions_this_turn_for_profile'):
        ...
        if updated_profile != previous_profile_for_comparison:
          ...
          # FIXED: Only show debug messages in debug mode
          if debug_mode:
